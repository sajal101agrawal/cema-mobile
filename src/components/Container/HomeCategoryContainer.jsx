...
const HomeCategoryContainer = ({ item }) => {
  const navigation = useNavigation();

  const imageUrl = useGenerateImageUrl(
    { imaePath: item.image_path, imageName: item.image },
    [item]
  );
...
export default memo(HomeCategoryContainer);

const styles = StyleSheet.create({
  listContainerStyle: {
    width: DEVICE_STYLES.SCREEN_WIDTH * 0.28,
    height: undefined,
    aspectRatio: 1,
    // alignItems : 'center',
