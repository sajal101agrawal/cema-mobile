...
import { LIGHT_BLUB_PRODUCT, BLUB_PRODUCT, WATCH_IMAGE } from '../../assets/images'
...
const ProductDetailSwiper = ({ imageArr = [], imagePath = '' }) => {
    ...
    const getDisplayImageUrl = (item) => imagePath === null ? item : `${imagePath}/${item}`;
    ...
    const ListPagination = useCallback(() => {
        return(
            <View style={styles.paginationContainer}>
                {
                    imageArr.map((_, index) => <View key={`${index}`} style={[styles.paginationDotStyle,{ backgroundColor : index === currentIndex ? Colors.SECONDARY_COLOR : Colors.WHITE, borderWidth : index === currentIndex ? 0 : 1 }]} />)
                }
            </View>
        )
    }, [currentIndex]);
    ...
    return (
        <>
            <FlatList
                ...
                contentContainerStyle={{ flexGrow: 1, backgroundColor : 'grey' }}
...
export default memo(ProductDetailSwiper);
